...
interface Step10Props {
  data: Step10Data;
  updateData: (data: Partial<Step10Data>) => void;
  onValidityChange: (isValid: boolean) => void;
}

const Step10CommercialEvaluation: React.FC<Step10Props> = ({
  data,
  updateData,
  onValidityChange,
}) => {
  useEffect(() => {
    ...
    const isValid = validateDocuments(data.commercialDocuments);
    onValidityChange(isValid);
  }, [data.commercialDocuments, onValidityChange]);
  ...
  const handleDocumentUpdate = (
    ...
    const currentDocs =
      data.commercialDocuments || INITIAL_DATA.step10.commercialDocuments;
    ...
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
      <div className="bg-blue-600 p-4 text-white">
        ...
        <p className="text-blue-100 opacity-90 text-sm">
          Evaluasi aspek komersial dan kewajaran harga.
        </p>
      </div>
      ...
      <div className="p-5 space-y-8">
        <div className="space-y-6">
          <h3 className="text-lg font-semibold text-slate-800 border-b pb-2">
            Dokumen Evaluasi Komersial
          </h3>
          <div className="space-y-4">
            {PROJECT_CONSTANTS.STEP10_DOCUMENTS.map((doc) => {
              const item = data.commercialDocuments?.[doc.key] ||
                INITIAL_DATA.step10.commercialDocuments?.[doc.key] || {
                  isActive: false,
                  existence: null,
                  suitability: null,
                  catatan: "",
...
export default Step10CommercialEvaluation;
